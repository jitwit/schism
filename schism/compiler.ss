...
(library (schism compiler)
    ...
    (let ((tag (car def)))
      ...
      (let ((name (definition-name def)))
        (if (function-definition? def)
            (let ((args (function-formals def)))
              (add-env name
                       (lambda ()
                         (let ((args (map rename-var args)))
                           `(lambda ,args (call ,name . ,(lexical-refs args)))))
                       env))
            ;; (add-env name `(lambda () (const ,(caddr def))) env)
            (error 'add-top-level "not a function definition")))))
  ...
  ;; function = (define (fun formals) body)
  ...
  (define (parse-functions definitions env)
    (map (lambda (fn) (parse-function fn env)) definitions))
